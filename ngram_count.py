...
def ngram_is_valid(ngram):
    ...
    if first in ch_stopwords or last in ch_stopwords: return False
    if any( num in first or num in last for num in '0123456789'): return False
    if any( symbol in word for word in ngram for symbol in eng_symbols): return False
    if any( symbol in word for word in ngram for symbol in ch_symbols): return False
    return True
...
if __name__ == '__main__':
    ...
    print '\n'.join( u'{}\t{}'.format(' '.join(ngram), count) for ngram, count in ngram_counter.most_common()).encode('utf-8')  